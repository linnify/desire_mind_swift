...
//  DesireMind
//
//  Created by Diana Dobocan on 26/04/2020.
...
//        let jsonEncoder = JSONEncoder()
    ...
    func delete (_ id: Int, completion: @escaping (Error?) -> ()) {
        let endpoint = "\(self.endpoint)/\(id)"
        self.performRequest(endpoint: endpoint, method: .delete, parameters: nil, completion: {(_: T?, error) in completion(error)})
    }
    ...
    private func performRequest<U: Codable>(endpoint: String, method: HTTPMethod, parameters: U?, completion: @escaping(U?, Error?) -> ()) {
        ...
        AF.request(url, method: method, parameters: parameters, encoder: JSONParameterEncoder.default, headers: headers, interceptor: nil).response { response in
                ...
                do {
                    ...
                    // for data fields jsonDEcoder.dateDecodingStrategy
